...
class NativeAudioSource {
	...
	public function init():Void {
		...
		if (disposed = (handle = AL.createSource()) == null) {
			return;
		}
		...
		if (stream = vorbisFile != null) {
			...
			//var constructor = /*bitsPerSample == 8 ? Int8 : Int16*/Int;
			for (i in 0...STREAM_NUM_BUFFERS) {
				buffers.push(AL.createBuffer());
				bufferDatas.push(new lime.utils.UInt8Array(STREAM_BUFFER_SIZE));//new ArrayBufferView(STREAM_BUFFER_SIZE, constructor));
	...
	public function setCurrentTime(value:Float):Float {
		...
		if (playing) {
			...
			if (completed = timeRemaining <= 0) complete();
				...
				AL.sourcePlay(handle);
				/*if (AL.getSourcei(handle, AL.SOURCE_STATE) != AL.PLAYING) {
					var p = getPitch(), g = getGain();
					dispose();
					init();
					setPitch(p); setGain(g); setLoops(getLoops());
