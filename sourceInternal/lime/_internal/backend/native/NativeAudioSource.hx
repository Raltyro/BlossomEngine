...
class NativeAudioSource {
	...
	public function init() {
		...
		if (loopPointsSupported && loopPoints == null) loopPoints = [0, 0];
	...
	function readToBufferData(data:ArrayBufferView, currentPCM:Int64):Int {
		...
		while (total < bufferLength) {
			...
			if (result == Vorbis.HOLE) continue;
				...
				if ((wasEOF == (wasEOF = true)) || (streamEnded = loops <= streamLoops)) break;
	...
	function timer_onRun() {
		...
		if (streamLoops > 0) {
			if ((loops -= streamLoops) < 0) loops = 0;
			streamLoops = 0;
			completeTimer = resetTimer(completeTimer, (getLength() + parent.offset - loopTime) / pitch - 50, timer_onRun);
		}
	...
	function updateCompleteTimer() {
		if (playing) {
			...
			if (timeRemaining > 50) completeTimer = resetTimer(completeTimer, timeRemaining - 50, timer_onRun);
			else {
	...
	public function setLoopTime(value:Float):Float {
		...
		if ((loopPoints[0] = Std.int(getRealLength() / 1000 * sampleRate)) >= samples) loopPoints[0] = samples - 1;
		updateLoopPoints();
