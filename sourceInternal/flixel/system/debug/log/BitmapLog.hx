...
class BitmapLog extends Window
{
	...
	public function new()
	{
		...
		canvas = new Bitmap(bl.util.BitmapDataUtil.create(Std.int(width), Std.int(height - 15)));
		canvas.x = 0;
		canvas.y = 15;
		addChild(canvas);
	...
	override function resize(width:Float, height:Float):Void
	{
		...
		{
			bl.util.BitmapDataUtil.resize(canvas.bitmapData, canvasWidth, canvasHeight);
	...
	function drawCanvas()
	{
		...
		matrix.setTo(zoom, 0, 0, zoom,
			(canvasBmd.width / 2) - (bitmap.width * zoom / 2) + canvasOffset.x,
			(canvasBmd.height / 2) - (bitmap.height * zoom / 2) + canvasOffset.y
		);

		//canvasBmd.fillRect(canvasBmd.rect, FlxColor.TRANSPARENT);
		bl.util.BitmapDataUtil.clear(canvasBmd, FlxColor.TRANSPARENT);
		//bl.util.BitmapDataUtil.draw(canvasBmd, bitmap, matrix);
