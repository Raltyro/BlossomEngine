...
class FlxText extends FlxSprite
{
	...
	function regenGraphic():Void
	{
		...
		if (textField.textHeight != 0 && (oldWidth != newWidth || oldHeight != newHeight))
		{
			if (_frame == null) {
				...
				frames = FlxGraphic.fromBitmapData(BitmapDataUtil.create(newWidth, newHeight), true, key).imageFrame;
				...
				BitmapDataUtil.resize(graphic.bitmap, newWidth, newHeight);
				BitmapDataUtil.clear(graphic.bitmap);
				graphic.bitmap = graphic.bitmap;
				frames.frames[0].sourceSize.set(newWidth, newHeight);
				frames.frames[0].frame = frames.frames[0].frame.set(0, 0, newWidth, newHeight);
			}

			if (_hasBorderAlpha) {
				if (_borderPixels == null) _borderPixels = BitmapDataUtil.create(newWidth, newHeight);
				else {
					BitmapDataUtil.resize(_borderPixels, newWidth, newHeight);
					BitmapDataUtil.clear(_borderPixels);
				}
			}
		...
		else // Else just clear the old buffer before redrawing the text
		{
			BitmapDataUtil.clear(graphic.bitmap);
			if (_hasBorderAlpha) {
				if (_borderPixels == null) _borderPixels = BitmapDataUtil.create(frameWidth, frameHeight);
				else BitmapDataUtil.clear(_borderPixels);
			}
		}
	...
	override function calcFrame(force = false):Void {
	...
	inline function applyBorderTransparency()
	{
		...
		//BitmapDataUtil.draw(graphic.bitmap, _borderPixels);
