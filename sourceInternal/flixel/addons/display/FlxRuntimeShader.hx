...
class FlxRuntimeShader extends FlxGraphicsShader
{
	 ...
	 */
	public function new(?fragmentSource:String, ?vertexSource:String, ?version:String):Void
	{
		if (version != null && version.length > 0) __glVersion = version;
	...
	{
		if (value != null) value = value.replace("#pragma header", buildGLSLHeaders(__glVersion) + __glFragmentHeaderRaw).replace("#pragma body", __glFragmentBodyRaw);
	...
	{
		if (value != null) value = value.replace("#pragma header", buildGLSLHeaders(__glVersion) + __glVertexHeaderRaw).replace("#pragma body", __glVertexBodyRaw);
