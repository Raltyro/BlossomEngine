...
class FlxAnimateFrames extends FlxAtlasFrames
{
	...
	public static function fromSpriteMap(Path:FlxSpriteMap, ?Image:FlxGraphicAsset):FlxAtlasFrames
	{
		...
		var f = findImage(Image, Path is String ? (haxe.io.Path.addTrailingSlash(haxe.io.Path.directory(Path)) + json.meta.image) : null);
	...
	public static function fromSparrow(Path:FlxSparrow, ?Image:FlxGraphicAsset):FlxAtlasFrames
	{
		...
		var f = findImage(Image, Path is String ? (haxe.io.Path.addTrailingSlash(haxe.io.Path.directory(Path)) + data.get("imagePath")) : null);
