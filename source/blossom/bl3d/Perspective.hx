...
class Perspective implements IFlxDestroyable {
	...
	public function applyShaderParameters(shader:Shader) {
		ShaderUtil.setParameterMatrix3D(shader.data.projectionMatrix, projectionMatrix);
		ShaderUtil.setParameterMatrix3D(shader.data.viewMatrix, viewMatrix);
	}
	...
	public function updateViewMatrix() {
		if (useLookAt) {
			if (_forward == null) _forward = Vector3.get();
			if (_right == null) _right = Vector3.get();
			if (_up == null) _up = Vector3.get();

			_forward.copyFrom(lookVector).subtract(x, y, z).normalize();
			_right.copyFrom(upVector).crossProduct(_forward).normalize();
			_up.copyFrom(_forward).crossProduct(_right);
