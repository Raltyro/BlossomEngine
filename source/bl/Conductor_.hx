...
@:structInit
class TimeChange {
	// this is actually songPosition, it's just off to name it "songPosition"
...
class StopChange {
	...
	@:optional public var songPosition:Null<Float>;
}
...
class Conductor {
	...
	public var onStopChange(default, null):FlxSignal = new FlxSignal();
	...
	public var oldSongPosition(default, null):Float = 0;
	...
	public var stopChanges:Array<StopChange> = [];
	...
	public var currentStopChange(get, never):Null<StopChange>;
	var currentStopChangeIdx:Int = -1;
	function get_currentStopChange() return stopChanges[currentStopChangeIdx];
	...
	public function update(?sound:flixel.util.typeLimit.OneOfTwo<FlxSound, Float>, rate = 1.0, ?forceDispatch:Bool, applyOffsets = true) {
		if (sound != null || FlxG.sound.music != null) {
			...
			time = sound == null ? FlxG.sound.music.time : (snd != null ? snd.time : cast sound);
			if (applyOffsets) time -= (appliedOffset = offset * (snd != null ? snd.getActualPitch() : 1.0) * rate);
			else appliedOffset = null;
		}

		oldSongPosition = Math.isNaN(songPosition) ? 0 : songPosition;
		if ((currentStopChangeIdx = getTimeInStopChangeIdx(time, currentStopChangeIdx)) >= 0)
			songPosition = Math.max(time - stopChange.time - currentStopChange.duration, 0) + currentStopChange.songPosition;
		else
			songPosition = time;
		...
		final oldTimeChangeIdx = currentTimeChangeIdx;
		oldBeat = currentBeat;
	...
	public function getTimeInStopChangeIdx(time:Float, idx:Int = -1):Int {
		if (stopChanges.length == 0) return -1;
		else if (stopChanges[idx = Math.floor(FlxMath.bound(idx, 0, stopChanges.length - 1))].time > time) {
			while (--idx >= 0) if (time > stopChanges[idx].time) return idx;
			return -1;
		}
		else {
			for (i in idx...stopChanges.length) if (stopChanges[i].time > time) return i - 1;
			return stopChanges.length - 1;
		}
	}
	...
	public function getTimeWithIdxInBPM(time:Float, idx:Int):Float {
		var timeChange = timeChanges[idx];
		if (timeChange.continuous && time < timeChange.endTime && idx > 0) {
			var prevBPM = timeChanges[idx - 1].bpm;
			if (time <= timeChange.time) return prevBPM;

			var ratio = (time - timeChange.time) / (timeChange.endTime - timeChange.time);
	...
	public function getBeatsWithIdxInBPM(beatTime:Float, idx:Int):Float {
		...
		if (timeChange.continuous && idx > 0) {
			...
			var endBeatTime = timeChange.beatTime + (timeChange.endTime - timeChange.time) * (timeChange.bpm - prevBPM) / Math.log(timeChange.bpm / prevBPM) / 240000 * getDenominator(timeChange);
	...
	public function getMeasuresInBPM(measureTime:Float):Float {
		var idx = getMeasuresInChangeIdx(measureTime);
	...
	public function getStepsInBPM(stepTime:Float):Float {
		var idx = getStepsInChangeIdx(stepTime);
		if (idx == -1) return DEFAULT_TIMECHANGE.bpm;
		else {
			var timeChange = timeChanges[idx];
	...
	public function getTimeInBeats(time:Float):Float {
		var idx = getTimeInChangeIdx(time);
	...
	public function getBeatsWithBPMInTime(beatTime:Float, idx:Int, bpm:Float):Float {
		...
		if (timeChange.continuous && beatTime > timeChange.beatTime && idx > 0) {
			var prevBPM = timeChanges[idx - 1].bpm;
			var time = timeChange.time + (beatTime - timeChange.beatTime) / (bpm - prevBPM) * Math.log(bpm / prevBPM) * 240000 / getDenominator(timeChange);
			if (time > timeChange.endTime)
	...
	public function getBeatsInTime(beatTime:Float):Float {
		var idx = getBeatsInChangeIdx(beatTime);
	...
	public function getMeasuresInTime(measureTime:Float):Float {
		...
		if (idx < 1) return measureTime * getMeasureLength(timeChanges[idx]);
			...
			var timeChange = timeChanges[idx];
			var beatTime = timeChange.beatTime + (measureTime - timeChange.measureTime) * getNumerator(timeChange);
	...
	public function getStepsInTime(stepTime:Float):Float {
		...
		if (idx < 1) return stepTime * getStepLength(timeChanges[idx]);
			...
			var timeChange = timeChanges[idx];
			var beatTime = timeChange.beatTime + (stepTime - timeChange.stepTime) / getTuplet(timeChange);
			return getBeatsWithBPMInTime(beatTime, idx, getBeatsWithIdxInBPM(beatTime, idx));
		}
	}
	...
	public function mapStopChanges(stopChanges:Array<StopChange>, updateAfter:Bool = true) {
